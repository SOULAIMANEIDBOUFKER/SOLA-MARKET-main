import { motion } from "framer-motion";
import { useCartStore } from "../stores/useCartStore";
import { Link } from "react-router-dom";
import { MoveRight } from "lucide-react";
import { loadStripe } from "@stripe/stripe-js";
import axios from "../lib/axios";
import { useTranslation } from "react-i18next";

// ✅ publishable key from Vite env (must match the same Stripe account as STRIPE_SECRET_KEY)
const stripePromise = loadStripe(import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY);

const OrderSummary = () => {
  const { t } = useTranslation();
  const { total, subtotal, coupon, isCouponApplied, cart } = useCartStore();

  const savings = subtotal - total;

  const handlePayment = async () => {
    try {
      // Guard: env key must exist
      if (!import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY) {
        console.error("Missing VITE_STRIPE_PUBLISHABLE_KEY in frontend/.env");
        return;
      }

      const stripe = await stripePromise;

      const res = await axios.post("/payments/create-checkout-session", {
        products: cart,
        couponCode: coupon ? coupon.code : null,
      });

      // backend returns: { id: session.id, totalAmount: ... }
      const session = res.data;

      if (!session?.id) {
        console.error("No session id returned from backend:", session);
        return;
      }

      const { error } = await stripe.redirectToCheckout({ sessionId: session.id });
      if (error) console.error("Stripe redirect error:", error.message);
    } catch (err) {
      console.error("Checkout error:", err);
    }
  };

  return (
    <motion.div
      className="space-y-4 rounded-lg border border-gray-700 bg-gray-800 p-4 shadow-sm sm:p-6"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5 }}
    >
      <p className="text-xl font-semibold text-emerald-400">
        {t("cart.orderSummary")}
      </p>

      <div className="space-y-4">
        <div className="space-y-2">
          <dl className="flex items-center justify-between gap-4">
            <dt className="text-base font-normal text-gray-300">
              {t("cart.originalPrice")}
            </dt>
            <dd className="text-base font-medium text-white">
              ${subtotal.toFixed(2)}
            </dd>
          </dl>

          {savings > 0 && (
            <dl className="flex items-center justify-between gap-4">
              <dt className="text-base font-normal text-gray-300">
                {t("cart.savings")}
              </dt>
              <dd className="text-base font-medium text-emerald-400">
                -${savings.toFixed(2)}
              </dd>
            </dl>
          )}

          {coupon && isCouponApplied && (
            <dl className="flex items-center justify-between gap-4">
              <dt className="text-base font-normal text-gray-300">
                {t("cart.coupon")} ({coupon.code})
              </dt>
              <dd className="text-base font-medium text-emerald-400">
                -{coupon.discountPercentage}%
              </dd>
            </dl>
          )}

          <dl className="flex items-center justify-between gap-4 border-t border-gray-600 pt-2">
            <dt className="text-base font-bold text-white">
              {t("cart.total")}
            </dt>
            <dd className="text-base font-bold text-emerald-400">
              ${total.toFixed(2)}
            </dd>
          </dl>
        </div>

        <motion.button
          className="flex w-full items-center justify-center rounded-lg bg-emerald-600 
          px-5 py-2.5 text-sm font-medium text-white hover:bg-emerald-700 
          focus:outline-none focus:ring-4 focus:ring-emerald-300"
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          onClick={handlePayment}
        >
          {t("cart.proceedToCheckout")}
        </motion.button>

        <div className="flex items-center justify-center gap-2">
          <span className="text-sm font-normal text-gray-400">
            {t("cart.or")}
          </span>
          <Link
            to="/"
            className="inline-flex items-center gap-2 text-sm font-medium 
            text-emerald-400 underline hover:text-emerald-300 hover:no-underline"
          >
            {t("cart.continueShopping")}
            <MoveRight size={16} />
          </Link>
        </div>
      </div>
    </motion.div>
  );
};

export default OrderSummary;
